...
echo "Warning: Do not use on production systems"
...
set -e
ES_CONF_FILE="./config/elasticsearch.yml"
ES_BIN_DIR="./bin"
ES_PLUGINS_DIR="./plugins"
ES_LIB_PATH="./lib"
SUDO_CMD=""
...
if [ -d "$ES_PLUGINS_DIR/search-guard-5" ]; then
  echo "Search Guard plugin already installed. Quit."
  exit -1
fi
...
echo "" >> $ES_CONF_FILE
echo "######## Search Guard Configuration ########" >> $ES_CONF_FILE
...
#$SUDO_CMD "$ES_BIN_DIR/elasticsearch-plugin" remove search-guard-5 > /dev/null
$SUDO_CMD "$ES_BIN_DIR/elasticsearch-plugin" install -b com.floragunn:search-guard-5:$ES_VERSION-11  > /dev/null
...
echo "### After the cluster is up execute: "
echo $SUDO_CMD "$ES_PLUGINS_DIR/search-guard-5/tools/sgadmin.sh" -cd "$ES_PLUGINS_DIR/search-guard-5/sgconfig" -cn searchguard_demo -ks "$ES_CONF_DIR/kirk.jks" -ts "$ES_CONF_DIR/truststore.jks" -nhnv
